...
layout (binding=2) uniform sampler2DMS btexms;
...
void main() {
	if (msaa == 0) {
		...
		vec2 texPos = vec2((pos.x + 1) / 2,  (pos.y + 1) / 2) * vec2(sWidth, sHeight);
		...
		for (int y = 0; y < sampleNum; y++) {
			for (int x = 0; x < sampleNum; x++) {
				...
				float weight = (maxDistFromMid - length(offset)) / maxDistFromMid;
				vec2 samplePos = texPos + offset * offsetScale;
				
				vec4 texelColor = vec4(0);
				for (int i = 0; i < numSamples; i++) {
					texelColor += texelFetch(btexms, ivec2(samplePos.x, samplePos.y), i);
				}
				color += (texelColor / numSamples) * weight;
				totalWeight += weight;
			}
		}
